// ============================================
// DER DIE DAS APP - MAIN APPLICATION LOGIC
// ============================================
// Handles article practice sessions with button-based selection
// User selects der/die/das for each German noun
...
/**
 * Initialize the Der Die Das app on page load.
...
async function init() {
  ...
  // Create learning engine for derdiedas app
  engine = new LearningEngine('derdiedas');
...
/**
 * Display a word card with article selection buttons.
 * Shows only the German noun, user must select der/die/das.
 *
...
function showCard(word) {
  ...
  // Reset card styling (neutral until answer)
  card.className = 'card';

  // Build card front HTML with article buttons
  card.innerHTML = `
    <div class="card__front">
      <div class="card__word">${word.de}</div>

      <div class="article-buttons">
        <button class="article-btn article-btn--der" data-article="der">der</button>
        <button class="article-btn article-btn--die" data-article="die">die</button>
        <button class="article-btn article-btn--das" data-article="das">das</button>
      </div>

      <button class="dont-know-btn" id="dont-know">I don't know</button>
    </div>
  `;

  // Attach event listeners to article buttons
  document.querySelectorAll('.article-btn').forEach(btn => {
    btn.addEventListener('click', () => checkAnswer(btn.dataset.article));
  });

  // "I don't know" button counts as incorrect
  document.getElementById('dont-know').addEventListener('click', () => checkAnswer(null));
...
/**
 * Check user's article selection and provide feedback.
 *
 * @param {string|null} selectedArticle - User's selected article or null for "I don't know"
 */
function checkAnswer(selectedArticle) {
  ...
  showCardBack(word, correct, selectedArticle);
}
 ...
 * @param {string|null} selectedArticle - User's selected article
 */
function showCardBack(word, correct, selectedArticle) {
  const card = document.getElementById('card');

  // Get gender class for color coding
  const genderClass = getGenderClass(word.article);

  // Apply gender color to card
  card.classList.add(`card--${genderClass}`);
  ...
  setTimeout(() => {
    ...
    if (selectedArticle === null) {
      ...
      userAnswerText = selectedArticle;
    }
      ...
      <div class="card-back">
        <div class="card-back__header">
          ...
          <div class="card-back__result ${correct ? 'correct' : 'incorrect'}">
            ${correct ? '✓ Correct' : `✗ ${userAnswerText}`}
          </div>
        </div>
...
async function exportData() {
  try {
    const response = await fetch('/api/derdiedas/export');
    ...
    a.href = url;
    a.download = `derdiedas-backup-${todayISO()}.json`;
